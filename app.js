        ...
        messages: [
          {
            ...
            content: `
              ...
              Important Rules:
              1. Always asnwer in pointers.
              2. First point must explain the answer in detail.
              3. Keep the response insightful (within 3-4 sentences).
              4. Focus on explaining query response, key insights, trends, or recommendations.
            `,
          },
          {
            role: "user",
        ...
        messages: [
          {
            ...
            content: `
              ...
              5. When grouping or filtering by month or date, always use TO_CHAR("Order_Date", 'YYYY-MM') to format date correctly.
              6. Always add an alias when using a subquery in the FROM clause.
            `,
          },
              ...
              .map((col) => {
                ...
                if (dateColumns.includes(col)) {
                    ...
                    if (typeof dateValue === "string" && dateValue.match(/^\d{4}-\d{2}$/)) {
                      return `${col}: ${dateValue}`; // Correct for YYYY-MM format
                    } else if (dateValue && !isNaN(Date.parse(dateValue))) {
                      // Fallback for full date
